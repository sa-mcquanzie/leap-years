...
describe Leap do
  describe '#is_leap?' do
    ...
    it 'returns true when passed a random sample from a large array of known leap years' do
      @leap = Leap.new
      100.times do
        year = list_of_leap_years.sample
        expect(@leap.is_leap?(year)).to eq(true)
      end
    end
  end
